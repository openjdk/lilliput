  ...
  if (ac->is_clone_array() && ary_ptr != NULL) {
    ...
    if (bt == T_OBJECT) {
      ...
      if (offset != arrayOopDesc::base_offset_in_bytes(T_OBJECT)) {
        //assert(!UseCompressedClassPointers, "should only happen without compressed class pointers, offset: %ld, base offset: %d", offset, arrayOopDesc::base_offset_in_bytes(T_OBJECT));
