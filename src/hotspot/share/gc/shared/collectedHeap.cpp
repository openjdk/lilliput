  ...
  _gc_lastcause(GCCause::_no_gc)
{
  ...
  int header_size_bytes = arrayOopDesc::header_size_in_bytes(T_INT);
  assert(header_size_bytes  % sizeof(jint) == 0, "must be aligned to int");
  int header_size_ints = header_size_bytes / sizeof(jint);
  _filler_array_max_size = align_object_size((header_size_ints + max_len) / elements_per_word);
