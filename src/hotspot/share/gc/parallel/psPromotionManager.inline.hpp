  ...
  if (new_obj != nullptr) {
    ...
    if (lab != nullptr) {
      ...
      if (gc_tracer->should_report_promotion_in_new_plab_event()) {
        ...
        gc_tracer->report_promotion_in_new_plab_event(old_obj->safe_klass(), obj_bytes,
      ...
      if (gc_tracer->should_report_promotion_outside_plab_event()) {
        size_t obj_bytes = obj_size * HeapWordSize;
        gc_tracer->report_promotion_outside_plab_event(old_obj->safe_klass(), obj_bytes,
                                                       age, tenured);
      }
    }
  }
...
inline oop PSPromotionManager::copy_unmarked_to_survivor_space(oop o,
  ...
  size_t new_obj_size = o->safe_size();
