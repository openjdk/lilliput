...
void PhaseMacroExpand::expand_arraycopy_node(ArrayCopyNode *ac) {
  ...
  if (ac->should_copy_int_prefix()) {
    // clone - either instance or reflective array
    // assert ac->is_clonebasic
  }

  if (ac->is_clonebasic()) { // handled in clone_at_expansion
    tty->print_cr("expand_arraycopy_node: clone_at_expansion (clonebasic)");
    BarrierSetC2* bs = BarrierSet::barrier_set()->barrier_set_c2();
    bs->clone_at_expansion(this, ac);
    return;
  } else if (ac->is_copyof() || ac->is_copyofrange() || ac->is_clone_oop_array()) { // should never be true, add assertion
